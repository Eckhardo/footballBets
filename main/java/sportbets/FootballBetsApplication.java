...
public class FootballBetsApplication {
    ...
    private void execute() {
        ...
        for (Spieltag spTagHinn : spieltagHin) {
            TippConfig tippConfig = new TippConfig(spTagHinn, compMemb, buliModus);
            spTagHinn.setTippConfig(tippConfig);
            spieltagRepo.save(spTagHinn);

        }
        for (Spieltag sptagRueck : spieltagRueck) {
            TippConfig tippConfig = new TippConfig(sptagRueck, compMemb, buliModus2);
            sptagRueck.setTippConfig(tippConfig);
            spieltagRepo.save(sptagRueck);
        }
